export default {
  header: {
    ...
    quickStart: '快速开始',
    phoneTitle: '确认修改手机号吗？',
    phoneContent: '点击确认或者回车即可修改',
    phoneError: '请输入正确的手机号',
    phoneSuccess: '绑定成功',
    phoneHover: '仅做用户标识作用, 不会对用户信息做任何收集',
  },
  common: {
    type: 'zh',
    ...
    upFailed: '上传失败',
    problemPlaceholder:
      "请输入问题(300字以内), 在文本开头 '@<标签1>  @<标签2>' 可锁定检索范围",
    errTip: '输入内容不能为空',
    uploadFile: '上传文件',
    ...
    rerankScoreThresholdLabel: '检索增强相关性阈值',
    rerankScoreThresholdLabelDescription: '检索增强时，相关性低于此阈值则丢弃',
  ...
  home: {
    ...
    require1:
      '可批量上传, 单个文档小于30M, 单张图片小于5M, 文件总大小不得超过125M',
    emptyText: '暂无数据',
    inputQa: '录入问答',
  ...
  bots: {
    ...
    creationTime: '创建时间',
    charLimit: '字数超出限制',
    roleSetPlaceholder: `- 你是一个耐心、友好、专业的机器人，能够回答用户的各种问题。
- 根据知识库内的检索结果，以清晰简洁的表达方式回答问题。
- 不要编造答案，如果答案不在经核实的资料中或无法从经核实的资料中得出，请回答“我无法回答您的问题。”
（或者您可以修改为：如果给定的检索结果无法回答问题，可以利用你的知识尽可能回答用户的问题。)`,
    welcomeMsgPlaceholder: '您好，我是您的专属机器人，请问有什么可以帮您呢？',
  },
  progress: {
