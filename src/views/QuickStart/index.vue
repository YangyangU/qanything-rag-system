              ...
              <div class="ai-content">
                <div class="ai-right">
                  <p
                    class="question-text"
                    :class="[
                      !item.source.length && !item?.picList?.length
                        ? 'change-radius'
                        : '',
                    ...
                    >
                      ...
                      >
                        <p v-show="sourceItem.file_name" class="control">
                          <span class="tips"
                            >{{ common.dataSource }}{{ sourceIndex + 1 }}:</span
                          >
                          <a
                          ...
                          <span
                            ...
                            :class="[
                              'file',
                              checkFileType(sourceItem.file_name)
                                ? 'filename-active'
                                : '',
                            ]"
        ...
        <div class="question">
          ...
          <div class="send-box">
            ...
            <div class="send-action">
              <TextCountLimit
                style="text-align: start !important; margin-left: 20px"
                :content="question"
                :limit="300"
              />
              <a-popover>
                ...
                <span
                  :class="[
                    'question-icon',
                    showLoading || !chatId ? 'isPreventClick' : '',
                  ]"
                  @click="shareChat"
                >
              ...
              <a-popover>
                ...
                <span
                  :class="[
                    'question-icon',
                    showLoading || !kbId ? 'isPreventClick' : '',
                  ]"
                  @click="uploadFile"
                >
...
import TextCountLimit from '@/components/TextCountLimit.vue';

const { common, home } = getLanguage();

const { copy } = useClipboard();
const { QA_List, chatId, kbId, kbIdCopy, showLoading } =
  storeToRefs(useQuickStart());
const {
  addHistoryList,
  updateHistoryList,
  addChatList,
...
const { setChatSourceVisible, setSourceType, setSourceUrl, setTextContent } =
  useChatSource();
...
const fileBlockArr = computed<IFileListItem[]>(() => {
  const arr =
    historyList.value?.fileToBeSendList.concat(uploadFileListQuick?.value) ||
    [];
...
const deleteFile = async (file_id: string, kb_id: string) => {
  ...
  try {
    await resultControl(
      await urlResquest.deleteFile({ file_ids: [file_id], kb_id }),
    );
    message.success('删除成功');
    const index: number = uploadFileListQuick.value.findIndex(
    ...
    if (index !== -1) {
      ...
      historyList.value.fileToBeSendList =
        historyList.value.fileToBeSendList.filter(
          (item) => item.file_id !== file_id,
        );
    }
  } catch (e) {
...
const addQuestion = (q) => {
  QA_List.value.push({
    ...
    type: 'user',
    fileDataList: [
      ...fileBlockArr.value.filter((item) => item.status !== 'red'),
    ],
  });
  scrollBottom();
};
...
const beforeSend = async (title) => {
  try {
    ...
    const res: any = await resultControl(
      await urlResquest.createKb({ kb_name: title }),
    );
...
const send = async () => {
  ...
  if (question.value.match(/[^a-zA-Z\s]|\p{P}|\w+/gu).length > 300) {
    message.warn('输入内容不能超过300字');
    return;
  }
  ...
  const sendData = {
    ...
    only_need_search_results:
      chatSettingFormActive.value.capabilities.onlySearch,
  ...
  if (chatSettingFormActive.value.capabilities.onlySearch) {
    ...
    try {
      ...
      if (res.code === 200) {
        QA_List.value[QA_List.value.length - 1].answer = res?.source_documents
          .length
...
const showDetail = (item: IChatItem, index) => {
  item.source[index].showDetailDataSource =
    !item.source[index].showDetailDataSource;
};
