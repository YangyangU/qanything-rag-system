    ...
    <ul>
      ...
      <li class="phone">
        <a-popover>
          ...
          <a-input
            ...
            :status="phoneStatus"
            style="width: 100%"
            @blur="handleChangePhoneOnce"
            @pressEnter="handleChangePhoneOnce"
          >
            <template #prefix>
      ...
      <li class="toggle-button">
        <span
          ...
          @click="changLanguage('zh')"
          >中</span
        >
        <span class="line"></span>
        <span
          :class="[language === 'en' ? 'active' : '']"
          @click="changLanguage('en')"
          >En</span
        >
      </li>
      <li>
...
const { resetStore } = useQuickStart();
...
const confirmPhone = () => {
  ...
  phoneStatus.value = '';
  message.success(header.phoneSuccess);
  console.log(window.location.hash.split('/').pop());
  if (window.location.hash.split('/').pop() === 'edit') {
    window.location.href = '#/bots';
  } else if (window.location.hash.split('/').pop() === 'quickstart') {
    resetStore();
  }
  window.location.reload();
};
...
const showConfirm = () => {
  const prePhone = userInfo.phoneNumber;
  modal.confirm({
    title: header.phoneTitle,
    icon: h(ExclamationCircleOutlined),
    content: h('p', header.phoneContent),
...
const handleChangePhone = () => {
  const prePhone = userInfo.phoneNumber;
  if (phone.value === prePhone) return;
  else if (!checkPhoneRegex(phone.value)) {
    phoneStatus.value = 'error';
    message.error(header.phoneError);
  } else {
    showConfirm();
  }
};
