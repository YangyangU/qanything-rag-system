...
const computedPercent = () => {
  ...
  // 调整百分比，确保总和为 100%
  let totalPercent = Object.values(progressPercent.value).reduce(
    (acc, val) => acc + val,
    0,
  );
  ...
  progressLength.value.red =
    progressPercent.value.red + progressLength.value.green;
  progressLength.value.yellow =
    progressPercent.value.yellow + progressLength.value.red;
  progressLength.value.gray =
    progressPercent.value.gray + progressLength.value.yellow;
};
