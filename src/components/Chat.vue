...
const { QA_List, chatId, pageId, qaPageId, historyList } =
  storeToRefs(useHomeChat());
const { chatSettingFormActive } = storeToRefs(useChatSetting());
const { copy } = useClipboard();
const { addHistoryList, updateHistoryList, addChatList, clearChatList } =
  useHomeChat();
...
const send = async () => {
  ...
  const sendData = {
    ...
    rerank_score: chatSettingFormActive.value.rerankScoreThreshold,
  };
