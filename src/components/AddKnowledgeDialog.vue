...
<template>
  <Teleport to="body">
    ...
    >
      <div class="kb-name">
        <span class="label"
          ><span class="red">*</span> {{ common.newPlaceholder }}</span
        >
        <a-input
      ...
      <div v-show="showUpload" class="file">
        <span class="label"
          ><span class="red">*</span> {{ common.uploadFile }}</span
        >
        <div class="box">
          <div
            class="before-upload-box"
            :class="showUploadList ? 'uploading' : ''"
          >
            <input
              class="hide input"
              type="file"
              ...
              <div class="upload-text-box">
                ...
                <p v-if="language === 'zh'">
                  <span class="upload-text"
                    >{{ common.dragUrl
                    }}<span class="blue">{{ common.click }}</span></span
                  >
                </p>
                <p v-else>
                  <span class="upload-text"
...
const { modalVisible, modalTitle, fileList, urlList, knowledgeName } =
  storeToRefs(useKnowledgeModal());
...
const showUpload = computed(() => {
  return (
    (newId.value && modalTitle.value === '新建知识库') ||
  ...
  fileList.value.forEach(async (file: IFileListItem, index) => {
    if (file.status == 'loading') {
      try {
        ...
        if (
          +res.code === 200 &&
          res.data[0].status !== 'red' &&
          res.data[0].status !== 'yellow'
        ) {
...
const handleOk = async () => {
  ...
  if (modalTitle.value === '新建知识库') {
    ...
    if (knowledgeName.value !== currentKbName.value) {
      ...
      try {
        await resultControl(
          await urlResquest.kbConfig({
            kbId: newId.value,
            kbName: knowledgeName.value,
          }),
        );
        confirmLoading.value = false;
        reset();
