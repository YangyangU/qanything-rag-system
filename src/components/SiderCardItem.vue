...
<template>
  <div
    :class="[
      'card',
      chatId === cardData.historyId ? 'active' : '',
    ...
    <a-popover overlay-class-name="card-hover" placement="right">
      <template #content>
        <div class="tools-box">
          <ul>
            <li
              @click="manage({ kb_name: cardData.title, kb_id: cardData.kbId })"
            >
...
const { setShowDeleteModal, setCurrentId, setCurrentKbName, setDefault } =
  useKnowledgeBase();
