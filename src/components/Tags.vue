  ...
  <div ref="tagsContainerRef" class="tags-container">
    <template v-for="tag in tagState.tags.slice(0, 5)" :key="tag">
      ...
      <a-tooltip
        ...
        color="#fff"
        placement="topLeft"
        :get-popup-container="() => tagsContainerRef"
      >
        <template #title>
    ...
    <a-tooltip color="#fff" :get-popup-container="() => tagsContainerRef">
      <template #title>
        <span style="color: #666; user-select: text">
          支持 英文 汉字 数字
          下划线，最多添加100个标签，单个标签长度不能超过30，多个标签以 ,隔开
        </span>
      </template>
      <a-input
...
const handleClose = (tag: string) => {
  const newTags = tagState.value.tags
    .map((item) => item.tag)
