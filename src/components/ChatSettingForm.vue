    ...
    >
      <a-form-item :label="common.modelProviderLabel" name="modelType">
        ...
        >
          ...
          >
            {{
              item.modelName.length === 0
                ? common.customModelType
                : item.modelName
            }}
          </a-select-option>
        </a-select>
      ...
      >
        <a-input
          v-model:value="chatSettingForm.modelName"
      ...
      <a-form-item ref="apiBase" :label="common.apiBaseLabel" name="apiBase">
        <a-input
          v-model:value="chatSettingForm.apiBase"
          aria-autocomplete="none"
        />
      </a-form-item>
      <a-form-item
        ref="apiModelName"
        :label="common.apiModelNameLabel"
        name="apiModelName"
      >
        <a-input
      ...
      <div class="form-item-inline">
        <a-form-item ref="apiContextLength" name="apiContextLength">
          ...
          <div
            v-if="chatSettingForm.modelType === 'ollama'"
            class="content-length-ollama-tip"
          >
            您需要先
            <a
            ...
            然后返回前端进行
            <span style="color: #888; background: rgba(150, 150, 150, 0.1)">
              总Token数量选择
            </span>
          </div>
        </a-form-item>
      ...
      <div class="form-item-inline">
        <a-form-item ref="rerankScoreThreshold" name="rerankScoreThreshold">
          <template #label>
            <a-popover placement="topLeft">
              <template #content>
                <p>{{ common.rerankScoreThresholdLabelDescription }}</p>
              </template>
              <span>{{ common.rerankScoreThresholdLabel }}</span>
            </a-popover>
          </template>
          <a-slider
            v-model:value="chatSettingForm.rerankScoreThreshold"
            :min="0.01"
            :max="1"
            :step="0.01"
          />
        </a-form-item>
        <a-form-item name="rerankScoreThreshold">
          <a-input-number
            v-model:value="chatSettingForm.rerankScoreThreshold"
            :min="0"
            :max="1"
      ...
      <a-form-item :label="common.capabilitiesLabel" name="capabilities">
        <a-checkbox-group
          v-model:value="capabilitiesOptionsState"
          style="width: 100%"
        >
          <a-row>
            <a-col
              v-for="key in Object.keys(chatSettingForm.capabilities)"
              :key="key"
              :span="6"
            >
              <a-popover placement="topLeft">
      ...
      >
        <a-button
          type="primary"
          html-type="submit"
          style="margin-left: 0 !important; width: auto"
        >
          {{ common.saveModel }}
        </a-button>
      </a-form-item>
...
watch(
  ...
  () => {
    ...
    // 如果上下文长度>=2maxToken为 512
    if (apiContextTokenK.value >= 2 && chatSettingForm.value.maxToken < 512) {
      chatSettingForm.value.maxToken = 512;
    }
    // 如果总token>=4k，则分片为800
    if (apiContextTokenK.value >= 4 && chatSettingForm.value.chunkSize < 800) {
      chatSettingForm.value.chunkSize = 800;
    }
  },
);
...
const rules: Record<string, Rule[]> = {
  ...
  modelName: [
    {
      required: true,
      message: `${common.plsInput}${common.modelNameLabel}`,
      trigger: 'change',
    },
  ],
  apiKey: [
    {
      required: true,
      message: `${common.plsInput}${common.apiKeyLabel}`,
      trigger: 'change',
    },
  ],
  apiBase: [
    {
      required: true,
      message: `${common.plsInput}${common.apiBaseLabel}`,
      trigger: 'change',
    },
  ],
  apiModelName: [
  ...
  maxToken: [
    {
      required: true,
      message: `${common.plsInput}${common.maxTokenLabel}`,
      trigger: 'change',
    },
  ],
  chunkSize: [
    {
      required: true,
      message: `${common.plsInput}${common.chunkSizeLabel}`,
      trigger: 'change',
    },
  ],
  temperature: [
    {
      required: true,
      message: `${common.plsInput}temperature`,
      trigger: 'change',
    },
  ],
  top_P: [
    { required: true, message: `${common.plsInput}top_P`, trigger: 'change' },
  ],
  top_K: [
    { required: true, message: `${common.plsInput}top_K`, trigger: 'change' },
  ],
  rerankScoreThreshold: [
    {
      required: true,
      message: `${common.plsInput}rerankScoreThreshold`,
      trigger: 'change',
    },
  ],
};
...
const selectChange = (value: 'openAI' | 'ollama' | number) => {
  ...
  if (value === 'openAI') {
    chatSettingForm.value = {
      ...chatSettingConfigured.value.find(
        (item) => item.modelType === 'openAI',
      ),
    };
  } else if (value === 'ollama') {
    chatSettingForm.value = {
      ...chatSettingConfigured.value.find(
        (item) => item.modelType === 'ollama',
      ),
    };
    // nextTick(() => {
...
function initForm(settings?: any) {
  if (settings) {
    if (
      settings.modelType === 'openAI' &&
      openAISettingMap.has(settings.apiModelName as any)
    ) {
      openAIModelSelect(settings.apiModelName);
      const { apiContextLength } = openAISettingMap.get(
        settings.apiModelName as any,
      );
      if (settings.apiContextLength !== apiContextLength) {
        apiContextTokenK.value = settings.apiContextLength / 1024;
      }
    }
    chatSettingForm.value = { ...settings };
    return;
  }
  console.log(chatSettingConfigured.value);
  const activeForm = {
    ...chatSettingConfigured.value.find((item) => item.active === true),
  };
  // 如果是openai，就设置最大值
  if (
    activeForm.modelType === 'openAI' &&
    openAISettingMap.has(activeForm.apiModelName as any)
  ) {
    openAIModelSelect(activeForm.apiModelName);
    // 如果这个配置改过，就覆盖
    const { apiContextLength } = openAISettingMap.get(
      activeForm.apiModelName as any,
    );
