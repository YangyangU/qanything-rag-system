      ...
      <div class="select-kenoledge-comp">
        <div class="header">
          <a-input
            ...
            :placeholder="bots.search"
          >
            <template #prefix>
              <img
                class="search-icon"
                src="@/assets/bots/search.png"
                alt="search-icon"
              />
            </template>
          </a-input>
          <a-button class="btn" type="primary" @click="toCreatekb">{{
            bots.createKb
          }}</a-button>
        </div>
        <div class="content">
          <div
            ...
            v-for="item in knowledgeList.filter((item) =>
              regex.test(item.kb_nmame),
            )"
            :key="item.kbId"
          >
...
const { selectKnowledgeVisible, knowledgeList, curBot } =
  storeToRefs(useBots());
