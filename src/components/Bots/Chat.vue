<template>
  <div class="bots-chat-container">
    <div class="my-page">
      ...
      <div id="chat" class="chat" @wheel="(e) => handleScroll(e)">
        <ul id="chat-ul" ref="scrollDom">
          <div class="ai">
            <div class="content">
              ...
              <p
                class="question-text"
                v-html="botInfo.welcome_message || bots.welcomeMsgPlaceholder"
              ></p>
            </div>
          </div>
              ...
              <template v-if="item.source.length">
                ...
                >
                  <span v-if="language === 'zh'"
                    >找到了{{ item.source.length }}个信息来源：</span
                  >
                  <span v-else
                    >Found {{ item.source.length }} source of information</span
                  >
                  <SvgIcon
...
const isOnMsgScrollingUp = ref(false);
const isOnMsg = ref(false);
...
const handleScroll = (e) => {
  if (e.deltaY < 0 && isOnMsg.value) {
    isOnMsgScrollingUp.value = true;
  }
};
...
const send = async () => {
  ...
  fetchEventSource(apiBase + '/local_doc_qa/local_doc_chat', {
    ...
    onclose(e: any) {
      isOnMsg.value = false;
      isOnMsgScrollingUp.value = false;
      console.log('close', e);
      typewriter.done();
      ctrl.abort();
      showLoading.value = false;
