    ...
    >
      <div class="file">
        <div class="mode">
          <p>当前文件上传模式为：</p>
          <a-switch
            v-model:checked="isSoft"
            checked-children="soft"
            un-checked-children="strong"
          />
        </div>
        <div class="box">
...
const beforeFileUpload = async (file, index) => {
  return new Promise((resolve, reject) => {
    ...
    if (
      file.name &&
      acceptList.includes('.' + file.name.split('.').pop().toLowerCase())
    ) {
      // 根据文件类型设置大小限制
      const limit = file.type.startsWith('image/')
        ? fileSizeLimit.image
        : fileSizeLimit.document;
...
const fileChange = (e) => {
  ...
  setTimeout(() => {
    uploadFileList.value.length && upload();
  });
  e.target.value = '';
};
...
const upload = async () => {
  ...
  formData.append(
    'chunk_size',
    chatSettingFormActive.value.chunkSize.toString(),
  );
  // 上传模式，soft：文件名重复的文件不再上传，strong：文件名重复的文件强制上传
  formData.append('mode', isSoft.value ? 'soft' : 'strong');

  openNotification(0);

  try {
    const response = await fetch(apiBase + '/local_doc_qa/upload_files', {
      method: 'POST',
      body: formData,
    });
    ...
    if (data.code === 200) {
      if (data.data.length === 0) {
        ...
        handleErrorFiles(list, data.msg || common.upFailed);
        return;
      }
      ...
      openNotification(1);
      if (props.dialogType === 1) {
        updateUploadStatus(list, data.data);
      }
    } else {
    ...
    if (timer.value) clearTimeout(timer.value); // 清除旧的定时器
...
// 更新文件的上传状态
const updateUploadStatus = (fileList: IFileListItem[], responseData: any[]) => {
  fileList.forEach((item, index) => {
    let status = responseData[index].status;
    status = status === 'green' || status === 'gray' ? 'success' : 'error';

    uploadFileList.value[item.order] = {
      ...uploadFileList.value[item.order],
      status,
      file_id: responseData[index].file_id,
      bytes: responseData[index].bytes,
      errorText: common.upSucceeded,
    };
  });
};

// 处理错误文件列表
const handleErrorFiles = (fileList: IFileListItem[], errorMsg: string) => {
