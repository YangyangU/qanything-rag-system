...
export const useOptiionList = defineStore(
  'option-list',
  () => {
    const allDataSource = ref<IDataSource[]>([]);
      ...
      res?.details.forEach((item: any, index) => {
        dataSource.value.push({
          ...
          remark:
            item?.status === 'gray'
              ? ''
              : computedRemark(item?.msg, item?.status),
        });
      });
          ...
          if (item?.picUrlList) {
            await fetchImagesAsFiles(item?.picUrlList)
              .then((files) => {
                ...
                // 进行赋值操作或其他处理
                faqList.value[i].picUrlList = item?.picUrlList.map(
                  (img, index) => {
                    return {
                      uid: -index,
                      name: 'image',
    ...
    const getAllDetails = async () => {
      ...
      while (hasMore) {
        ...
        if (res.details && res.details.length > 0) {
          allDocuments = allDocuments.concat(res.details);
        }
